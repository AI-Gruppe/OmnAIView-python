...
parser.add_argument(
    "--logging", action="store_true", help="Enable terminal logging of stream values"
)
...
class DevDataClient(tk.Tk):
    def __init__(self):
        ...
        ttk.Button(bar, text="Connect to Websocket", command=self._connect_dialog).pack(
            side="left", padx=4, pady=4
        )
        # Record data : Data is saved in RAM until flushed
        self.rec_btn = ttk.Button(
            bar, text="● Record", command=self._toggle_recording, state="disabled"
        )
        ...
        self.analyse_btn = ttk.Button(
            bar, text="Analysis", command=self._run_analysis, state="disabled"
        )
    ...
    def _connect_dialog(self):
        ...
        ttk.Label(dlg, text="WebSocket-Server (ip:port):").grid(
        ...
        ttk.Label(dlg, text="Data source:").grid(
    ...
    def _device_dialog(self, devices: List[Device]):
        ...
        ttk.Entry(frm, textvariable=rate_var, width=8).grid(
        ...
        ttk.Button(
            ...
            command=partial(
                self._start_measurement, dlg, vars_, devices, rate_var, fmt_var
            ),
        ).pack(pady=8)
            ...
            async with connect(uri) as ws:
                ...
                try:
                    while not self.stop_event.is_set():
                        ...
                        values = [
                            val_dict.get(uid, float("nan")) for uid in self.active_uuids
                        ]
